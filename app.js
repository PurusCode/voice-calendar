...
function openInCalendar(task) {
  ...
  a.click(); // Мгновенно открывает системный календарь
  document.body.removeChild(a);
  URL.revokeObjectURL(url);
}

// ============================================
// 🔔 4. УВЕДОМЛЕНИЯ (ФОНОВЫЙ ПРОВЕРКА)
// ============================================
...
document.getElementById('btn-add').onclick = async () => {
  ...
  try {
    ...
    console.warn('Авто-открытие календаря заблокировано браузером');
  }
...
async function renderTasks() {
  ...
  container.innerHTML = tasks.map(t => {
    ...
    return `
      <div class="task-card" style="${t.notified ? 'opacity:0.5;text-decoration:line-through' : ''}">        <div class="task-info">
